...
pkgs = ['python3', 'pip3', 'php', 'ssh', 'pyngrok']
inst = True
for pkg in pkgs:
	present = which(pkg)
	if present == None:
...
/____  > \___  >\___  >|__|_ \ \___  >|__|
	...
	print(G + '[0_o]' + G + ' Modified by:' + R + ' CacucoH' + '\n')
...
def server():
	print(G + '[i]' + W + ' Starting ngrok')
	url = ngrok.connect(str(port_selection))
	print (G + "[" + C + "i" + G + "]" + C + " Tunnel url:", url)
	print('\n' + G + '[+]' + C + ' Port : '+ W + str(port_selection))
...
try:
	...
	tunnel_select()
	template_select()
	port_selection = int(input(G + '[i]' + C + ' Enter port value(recommended 8080): ' + W))
	if port_selection >= 65536 or port_selection <= 0:
		print(R + '[!] ' + C + 'Invalid input!')
		sys.exit()
	server()
	wait()
	main()
